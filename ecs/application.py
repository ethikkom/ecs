...
def getTextboxList():
    ...
    dir = os.path.join(os.path.dirname(__file__), 'static', '3rd-party', 'TextboxList')

    if os.path.exists(dir):
        print 'You already have TextboxList'
        return
    else:
        os.mkdir(dir)

    data = urlopen('http://devthought.com/?download=textboxlist-latest.zip').fp.read()
    zf = ZipFile(StringIO(data))

    src_prefix = 'TextboxList/Source/'
    filenames = [x[len(src_prefix):] for x in zf.namelist() if x.startswith(src_prefix) and len(x) > len(src_prefix)]
    ...
    for filename in filenames:
        ...
        with open(outfilename, 'wb') as outfile:
            outfile.write(zf.read('%s%s' % (src_prefix, filename)))
